...
$routes->group(CI_SITE_AREA, ['namespace' => '\Adnduweb\Ci4_diaporama\Controllers\Admin', 'filter' => 'apiauth'], function ($routes) {
    ...
    $routes->get('(:num)/(:any)/pages', 'AdminDiaporamaController::renderViewList', ['as' => 'page-index']);
    $routes->get('(:num)/(:any)/pages/edit/(:any)', 'AdminDiaporamaController::renderForm/$3');
    $routes->post('(:num)/(:any)/pages/edit/(:any)', 'AdminDiaporamaController::postProcess/$3');
    $routes->get('(:num)/(:any)/pages/add', 'AdminDiaporamaController::renderForm');
    $routes->post('(:num)/(:any)/pages/add', 'AdminDiaporamaController::postProcess');
});

//ADN HACK;
//$routes->group('', ['namespace' => '\Adnduweb\Ci4_diaporama\Controllers\Front'], function ($routes) {
...
if (service('Settings')->setting_activer_multilangue == true) {
    ...
    $routes->get($locale . '/(:segment)', 'FrontPagesController::show/$1', ['namespace' => '\Adnduweb\Ci4_diaporama\Controllers\Front']);
//});
